<template>
  <v-app-bar app color="red darken-1" dark fixed>
    <v-img
    ...
    >
        ...
        <v-tooltip bottom>
          <template v-slot:activator="{ on: tooltip }">
            ...
            >
              ...
              <v-badge
                :content="
                  newNotificationsCount == 0 ? '0' : newNotificationsCount
                "
                :value="true"
                color="red"
                overlap
          ...
          <span class="mt-6">{{
            newNotificationsCount == 0
              ? $vuetify.lang.t(
                  "$vuetify.SEM_NOTIFICACOES",
                  newNotificationsCount
                )
              : newNotificationsCount == 1
              ? $vuetify.lang.t(
                  "$vuetify.NOVA_NOTIFICACAO",
                  newNotificationsCount
                )
              : $vuetify.lang.t(
                  "$vuetify.NOVAS_NOTIFICACOES",
                  newNotificationsCount
                )
          }}</span>
        </v-tooltip>
      ...
      <v-card width="300" height="450" style="overflow-y: hidden">
        <v-list color="red darken-3" dark>
          <v-list-item>
            <v-list-item-content>
        ...
        <v-tabs v-model="tab" centered color="red accent-4" icons-and-text>
        ...
        <v-tabs-items v-model="tab">
          ...
          <v-tab-item value="tab-2">
            <v-list style="overflow-y: auto">
              <v-list-item v-for="perfil in user.perfis" :key="perfil">
    ...
    >
      ...
      <v-list>
        ...
        <v-list-item v-show="canShowShortcut()" @click="goTo('commercial')">
          <v-list-item-title>{{
            $vuetify.lang.t("$vuetify.COMERCIAL")
          }}</v-list-item-title>
        </v-list-item>
        ...
        <v-list-item v-show="canShowAdminLink()" @click="toProactivePage()">
          <v-list-item-title>{{
            $vuetify.lang.t("$vuetify.ADMINISTRACAO")
          }}</v-list-item-title>
        </v-list-item>
    ...
    <v-tooltip bottom>
      <template v-slot:activator="{ on }">
        <v-btn icon color="white" class="ml-3" @click="faq()">
    ...
    <v-tooltip bottom>
      <template v-slot:activator="{ on }">
        <v-btn icon color="white" class="ml-2 mr-0" @click="logout()">
...
export default {
  ...
  created: function () {
    this.$getUser().then((user) => {
      ...
      (function (i, s, o, g, r, a, m) {
        i[r] = { context: { id: "8551dbde3f168db249381597eea81c71" } };
        a = o;
        o = s.createElement(o);
        o.async = 1;
        o.src = g;
        ...
        document,
        "script",
        "https://js.huggy.chat/widget.min.js?v=8.0.0",
        "pwz"
      );
  ...
  methods: {
    removeNotification(notification, event) {
      ...
      this.$delete("/notificacao/delete", {
    ...
    viewNotification(notification) {
      this.$router.push({
        name: "Dashboard",
        params: { contract: notification.contrato },
      });

      setTimeout(() => {
        if (
          notification.tipo == "NOVA_NOTA_FISCAL" ||
          notification.tipo == "NOTA_FISCAL_CANCELADA"
        ) {
          if (!this.$vuetify.breakpoint.xs) this.$refs.invoiceChip.click();
          if (this.$vuetify.breakpoint.xs) this.$refs.invoiceItem.click();

          this.$root.$emit(
            notification.tipo == "NOVA_NOTA_FISCAL"
              ? "search-invoice"
              : "search-canceled-invoice",
            notification.mensagem
          );
        } else if (notification.tipo == "CIRCUITO_OFFLINE") {
          if (!this.$vuetify.breakpoint.xs) this.$refs.circuitChip.click();
          if (this.$vuetify.breakpoint.xs) this.$refs.circuitItem.click();

          this.$root.$emit("search-circuit", notification.mensagem);
        } else {
          if (!this.$vuetify.breakpoint.xs) this.$refs.issueChip.click();
          if (this.$vuetify.breakpoint.xs) this.$refs.issueItem.click();
      ...
      this.$get("/notificacao/visualizar", {
        notificationId: notification.id,
      }).then(() => {
